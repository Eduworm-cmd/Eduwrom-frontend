    ...
    <div className="bg-white rounded-md w-full">
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm">
        <div className="bg-slate-300 border-b border-gray-200">
          <div className="grid" style={{ gridTemplateColumns: `repeat(${columns.length + 2}, minmax(0, 1fr))` }}>
            {columns.map((col) => (
              <div key={col.key} className="p-4 text-sm font-semibold text-gray-700">
                {col.label}
              </div>
            ))}
            <div className="p-4 text-center text-sm font-medium text-gray-700">Actions</div>
            <div className="p-4 text-center text-sm font-medium text-gray-700">Status</div>
          </div>
        </div>
        ...
        <div className="bg-white max-h-82 overflow-y-auto">
          {currentItems.map((item) => (
            <div
              key={item.id}
              className="relative grid border-b border-gray-200 hover:bg-gray-50 transition-colors"
              style={{ gridTemplateColumns: `repeat(${columns.length + 2}, minmax(0, 1fr))` }}
            >
              {columns.map((col) => (
                <div key={col.key} className="p-4 text-gray-700 font-medium">
            ...
            page === "..." ? (
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-semibold ${
                  currentPage === page ? "bg-indigo-600 text-white" : "text-gray-900 ring-1 ring-gray-300 hover:bg-gray-50"
                }`}
              >
                {page}
              </button>
